...
```ts
import { OASModel, ResolveRefsInObj } from 'fets'
import type oas from './openapi'

// This will infer `User` from `components.schemas.User`
type User = OASModel<ResolveRefsInObj<typeof oas>, 'User'>
```
...
```ts
import { OASRequestParameters, ResolveRefsInObj } from 'fets'
import type oas from './openapi'

type AddUserInput = OASRequestParams<ResolveRefsInObj<typeof oas>, '/user', 'POST'>
```
...
import { OASResponseBody, ResolveRefsInObj } from 'fets'
import type oas from './openapi'

type UserByIdResponse = OASResponseBody<ResolveRefsInObj<typeof oas>, '/user/:id', 'POST', 200>
```
