...
to every request made by the client. This includes feTS request fields (`headers`, `query`,
`params`, `json`, …) as well as standard `RequestInit` options such as `credentials`.

Per-request values override global ones.
