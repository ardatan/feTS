...
Handlers receive a second `context` argument typed from `createRouter<TServerContext>()`. Use it to
pass platform-specific values (for example the underlying Node/`uWebSockets` request) or
request-scoped data from plugins.
...
const router = createRouter<AppContext>().route({
  method: 'GET',
  path: '/me',
  handler(_request, context) {
    return Response.json({ userId: context.userId ?? null })
  }
})

// `fetch(url, init?, ...ctx)` — context comes after the optional RequestInit
// `handle(request, context)` — context is the second argument
await router.fetch('http://localhost/me', {}, { userId: '42' })
```

Platform integrations often type `ServerContext` with the native request/response objects — see
[uWebSockets](/server/integrations/uwebsockets) for an example.
