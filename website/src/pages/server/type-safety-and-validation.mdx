...
- **Request Validation:** When schemas are built with TypeBox (`Type.*`), feTS validates the request
  and returns `400 Bad Request` if it is invalid. Plain JSON Schema objects provide typing and
  OpenAPI metadata only — they are **not** validated at runtime.
...
<Callout type="warning">
  **Runtime validation requires TypeBox.** Use `Type.Object(...)`, `Type.String()`, etc. from
  `fets` / `@sinclair/typebox`. Hand-written JSON Schema (`{ type: 'object', ... }`) still gives you
  TypeScript types and OpenAPI docs, but will not reject invalid requests at runtime.
</Callout>
...
With TypeBox schemas, feTS coerces query string values to the declared types before validation
(for example `"10"` → `10` for `Type.Number()`, and `"true"` → `true` for `Type.Boolean()`), so
handlers receive correctly typed values:
