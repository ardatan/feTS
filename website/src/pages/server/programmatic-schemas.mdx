...
headers: Type.Object({ authorization: Type.String() }), params: Type.Object({ id: Type.String({
format: 'uuid' }) }) } }, // Response type will be automatically inferred from the handler's return
type handler: request => { if (request.params.id !== EXPECTED_UUID) { return Response.json({
message: 'User not found' }, { status: 404 }) } return Response.json({ id: request.params.id, name:
'John Doe' }) } })
